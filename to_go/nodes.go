...
func (self *printer) fieldList(fields *ast.FieldList, isStruct, isIncomplete bool) {
	...
	if !hasComments && srcIsOneLine { // possibly a one-line struct/interface
		if len(list) == 0 { // no blank between keyword and {} in this case
...
func (self *printer) stmt(stmt ast.Stmt, nextIsRBrace bool) {
	...
	case *ast.SelectStmt:
		...
		body := s.Body
		if len(body.List) == 0 && !self.commentBefore(self.posFor(body.Closing)) { // print empty select statement w/o comments on one line
...
func (self *printer) genDecl(d *ast.GenDecl) {
	...
	if d.Indent.IsValid() { // group of parenthesized declarations
...
//
func (self *printer) adjBlock(headerSize int, sep whiteSpace, b *ast.BlockStmt) {
	if b == nil {
		return
