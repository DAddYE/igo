...
func SortImports(fset *token.FileSet, f *File) {
	for _, d := range f.Decls {
		...
		// Deduping can leave a blank line before the dedent; clean that up.
		if len(d.Specs) > 0 {
			lastSpec := d.Specs[len(d.Specs)-1]
			lastLine := fset.Position(lastSpec.Pos()).Line
			if dedentLine := fset.Position(d.Dedent).Line; dedentLine > lastLine+1 {
				fset.File(d.Dedent).MergeLine(dedentLine - 1)
			}
		}
	}
}
